...
--     process, -- function used to "process/consume" values from the qfer
...
process = function(premature, self, batch)
  ...
  if ok then -- success, set retry_delays to 1
    ...
    ngx_log(ERR, "failed to process entries: ", tostring(err))
    ...
    if batch.retries < self.retry_count then
      ...
      ngx_log(WARN, fmt("entry batch was already tried %d times, dropping it",
                    batch.retries))
    end
...
-- @param payload included in the queue
